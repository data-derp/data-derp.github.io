...
```
We could hypothetically write a variety of unit tests for, but we could further improve future development by breaking up the logic and testing each, ensuring that when new changes, it would be easy to pin-point where something has gone wrong.

To break down this logic into several testable parts, we'll first extract the filtering logic into its own function:
```python showLineNumbers
def filter_data(input_df: DataFrame) -> DataFrame:
    return input_df.filter((input_df.action == "StartTransaction") & (input_df.message_type == 2))
```
Note that the input AND output are both DataFrames.  Next we call the function via the [transform function](https://spark.apache.org/docs/3.2.0/api/python/reference/api/pyspark.sql.functions.transform.html). The [transform function](https://spark.apache.org/docs/3.2.0/api/python/reference/api/pyspark.sql.functions.transform.html) is powerful in that it allows us to chain custom Dataframe functions (which are in turn, independent and testable).

```python
df.transform(filter_data)
```

Lastly, our unit test provides a mock of the incoming data and tests ONLY that the expected data is returned, namely the number of resulting records and the values of significant columns.
...
def test_filter_data_unit(spark, f: Callable):
...
test_filter_data_e2e(df.transform(filter_data))
```
